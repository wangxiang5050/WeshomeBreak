...
- Phase 1 is static engraving (no fly-in animation). Staff Melody Scene engraves on the dark overlay via a transparent `WKWebView` (white/near-white notation ink, no light “print card”), with bottom inset so the score stays clear of skip/delay controls.
